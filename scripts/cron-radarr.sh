...
#
# Modos de uso:
#   cron-radarr.sh              → solo etiquetado (tag + clean via SSH)
#   cron-radarr.sh --hardlinks  → etiquetado + reconciliación de hardlinks
#
# Ejemplo de crontab:
#   0 3 * * * /usr/local/bin/cron-radarr.sh
#   0 5 * * * /usr/local/bin/cron-radarr.sh --hardlinks
#
# Configuración:
